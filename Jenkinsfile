    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists orders;"
    ...
    stage('Generate docker image for development') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep cloud-hashing-order | grep latest | awk '{ print $3 }'`
